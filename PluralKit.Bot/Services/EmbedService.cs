...
namespace PluralKit.Bot {
    public class EmbedService
    {
        ...
        public async Task<DiscordEmbed> CreateSystemEmbed(Context cctx, PKSystem system, LookupContext ctx)
        {
            ...
            {
                if (memberCount > 0)
                    eb.AddField($"Members ({memberCount})", $"(see `pm!system {system.Hid} list` or `pm!system {system.Hid} list full`)", true);
                else
                    eb.AddField($"Members ({memberCount})", "Add one with `pm!member new`!", true);
            }
