    ...
    public class ImportExport
    {
            ...
            await ctx.BusyIndicator(async () =>
            {
                using (var client = new HttpClient())
                {
                    ...
                    else if (ctx.System == null)
                    {
                        ...
                        await ctx.Reply($"{Emojis.Success} We've created a system based on the .json. Your system ID is `{result.System.Hid}`. Type `pm!system` for more information.");
                    }
                    else
                    {
                        // We already had a system, so show them what changed
                        await ctx.Reply($"{Emojis.Success} Updated {result.ModifiedNames.Count} members, created {result.AddedNames.Count} members. Type `pm!system list` to check!");
                    }
                }
            });
        }
        ...
        private async Task<DataFileSystem> ImportFromTupperbox(Context ctx, JObject json)
        {
            ...
            if (res.HadGroups || res.HadIndividualTags)
            {
                ...
                if (res.HadGroups)
                    issueStr += "\n- Proxymaster does not support member groups. Members will be imported without groups.";
                if (res.HadIndividualTags)
                    issueStr += "\n- Proxymaster does not support per-member system tags. Since you had multiple members with distinct tags, those tags will be applied to the members' *display names*/nicknames instead.";
