    ...
    public class MemberAvatar
    {
        ...
        private async Task AvatarClear(AvatarLocation location, Context ctx, PKMember target, MemberGuildSettings? mgs)
        {
            ...
            else
            {
                if (mgs?.AvatarUrl != null)
                    await ctx.Reply($"{Emojis.Success} Member avatar cleared. Note that this member has a server-specific avatar set here, type `pm!member {target.Reference()} serveravatar clear` if you wish to clear that too.");
                else 
        ...
        private async Task AvatarShow(AvatarLocation location, Context ctx, PKMember target, MemberGuildSettings? guildData)
        {
            ...
            if (string.IsNullOrEmpty(currentValue) || !canAccess)
            {
                ...
                if (location == AvatarLocation.Server)
                    throw new PKError($"This member does not have a server avatar set. Type `pm!member {target.Reference()} avatar` to see their global avatar.");
            }
            ...
            if (target.System == ctx.System?.Id)
                eb.WithDescription($"To clear, use `pm!member {target.Reference()} {cmd} clear`.");
