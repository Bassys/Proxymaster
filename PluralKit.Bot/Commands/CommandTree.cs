...
namespace PluralKit.Bot
{
    public class CommandTree
    {
		...
		public static Command ThankYou = new Command ("thankyou", "ty", "Thanks the bot!");
		...
		public static Command ExplainDID = new Command("DID", "OSDD", "Explains what DID/OSDD-1 is");
		public static Command ExplainTerms = new Command("Terms", "Terminology", "Links to a carrd with system terms");
		public static Command ExplainFlag = new Command("Flag", "Flag", "Explanation of the system flag");
        ...
        public static Command BlacklistShow = new Command("blacklist show", "blacklist view", "Displays the current proxy blacklist");
        ...
        public static Command Invite = new Command("invite", "invite", "Gets a link to invite Proxymaster to other servers");
        ...
        public Task ExecuteCommand(Context ctx)
        {
			...
			if (ctx.Match("stats"))
                return ctx.Execute<Misc>(Stats, m => m.Stats(ctx));
            if (ctx.Match("export"))
            ...
            if (ctx.Match("blacklist", "bl"))
                ...
                else if (ctx.Match("list", "show", "view"))
            ...
            return ctx.Reply(
                $"{Emojis.Error} Unknown command {ctx.PeekArgument().AsCode()}.");
        }
        ...
        {
            // Commands that have a system target (eg. pm!system <system> fronthistory)
        ...
        {
            // Commands that have a member target (eg. pm!member <member> delete)
        ...
        private async Task PrintCommandNotFoundError(Context ctx, params Command[] potentialCommands)
        {
            ...
            await ctx.Reply(
                $"{Emojis.Error} Unknown command `pm!{ctx.FullCommand().Truncate(100)}`. That is not a viable command.");
        }
        ...
        private async Task PrintCommandExpectedError(Context ctx, params Command[] potentialCommands)
        {
            var commandListStr = CreatePotentialCommandList(potentialCommands);
            await ctx.Reply(
                $"{Emojis.Error} That is not a viable command.");
        }
        
        private static string CreatePotentialCommandList(params Command[] potentialCommands)
        {
            return string.Join("\n", potentialCommands.Select(cmd => $"- **pm!{cmd.Usage}** - *{cmd.Description}*"));
        }
        ...
        private async Task PrintCommandList(Context ctx, string subject, params Command[] commands)
        {
            ...
            await ctx.Reply($"For a list of Proxymasters commands click here! https://proxymaster.carrd.co/#commands");
        }
