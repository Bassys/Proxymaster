    ...
    public class Groups
    {
        ...
        public async Task CreateGroup(Context ctx)
        {
            ...
            var eb = new DiscordEmbedBuilder()
                ...
                .AddField("View the group card", $"> pm!group **{newGroup.Reference()}**")
                .AddField("Add members to the group", $"> pm!group **{newGroup.Reference()}** add **MemberName**\n> pm!group **{newGroup.Reference()}** add **Member1** **Member2** **Member3** (and so on...)")
                .AddField("Set the description", $"> pm!group **{newGroup.Reference()}** description **This is my new group, and here is the description!**")
                .AddField("Set the group icon", $"> pm!group **{newGroup.Reference()}** icon\n*(with an image attached)*");
        ...
        public async Task RenameGroup(Context ctx, PKGroup target)
        {
            ...
            if (existingGroup != null && existingGroup.Id != target.Id) {
                ...
                if (!await ctx.PromptYesNo(msg))
                    throw new PKError("Group renaming cancelled.");
            }
        ...
        public async Task GroupDisplayName(Context ctx, PKGroup target)
        {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (ctx.System?.Id == target.System)
                    eb.WithDescription($"To change display name, type `pm!group {target.Reference()} displayname <display name>`.\nTo clear it, type `pm!group {target.Reference()} displayname -clear`.");
        ...
        public async Task GroupDescription(Context ctx, PKGroup target)
        {
            ...
            else if (!ctx.HasNext())
            {
                if (target.Description == null)
                    if (ctx.System?.Id == target.System)
                        await ctx.Reply($"This group does not have a description set. To set one, type `pm!group {target.Reference()} description <description>`.");
                    else
                    ...
                    await ctx.Reply(embed: new DiscordEmbedBuilder()
                        ...
                        .AddField("\u200B", $"To print the description with formatting, type `pm!group {target.Reference()} description -raw`." 
                                    + (ctx.System?.Id == target.System ? $" To clear it, type `pm!group {target.Reference()} description -clear`." : ""))
                        .Build());
            }
            else
            {
        ...
        public async Task GroupIcon(Context ctx, PKGroup target)
        {
            ...
            async Task ShowIcon()
            {
                if ((target.Icon?.Trim() ?? "").Length > 0)
                {
                    ...
                    {
                        eb.WithDescription($"To clear, use `pm!group {target.Reference()} icon -clear`.");
                    }
        ...
        public async Task ListSystemGroups(Context ctx, PKSystem system)
        {
            ...
            {
                if (system.Id == ctx.System?.Id)
                    await ctx.Reply("This system has no groups. To create one, use the command `pm!group new <name>`.");
                else
        ...
        public async Task ShowGroupCard(Context ctx, PKGroup target)
        {
            ...
            if (target.ListPrivacy.CanAccess(pctx))
            {
                if (memberCount == 0 && pctx == LookupContext.ByOwner)
                    ...
                    eb.AddField("Members (0)", $"Add one with `pm!group {target.Reference()} add <member>`!", true);
                else
                    eb.AddField($"Members ({memberCount})", $"(see `pm!group {target.Reference()} list`)", true);
            }
