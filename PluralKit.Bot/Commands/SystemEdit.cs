...
namespace PluralKit.Bot
{
    public class SystemEdit
    {
        ...
        public async Task Name(Context ctx)
        {
            ...
            if (newSystemName == null)
            {
                if (ctx.System.Name != null)
                    await ctx.Reply($"Your system's name is currently **{ctx.System.Name}**. Type `pm!system name -clear` to clear it.");
                else
                    await ctx.Reply("Your system currently does not have a name. Type `pm!system name <name>` to set one.");
                return;
            }
        ...
        public async Task Description(Context ctx) {
            ...
            if (newDescription == null)
            {
                if (ctx.System.Description == null)
                    await ctx.Reply("Your system does not have a description set. To set one, type `pm!s description <description>`.");
                    ...
                    await ctx.Reply(embed: new DiscordEmbedBuilder()
                        ...
                        .WithFooter("To print the description with formatting, type `pm!s description -raw`. To clear it, type `pm!s description -clear`. To change it, type `pm!s description <new description>`.")
                        .Build());
            }
            else
            {
        ...
        public async Task Tag(Context ctx)
        {
            ...
            } else if (!ctx.HasNext(skipFlags: false))
            {
                if (ctx.System.Tag == null)
                    await ctx.Reply($"You currently have no system tag. To set one, type `pm!s tag <tag>`.");
                else
                    await ctx.Reply($"Your current system tag is {ctx.System.Tag.AsCode()}. To change it, type `pm!s tag <tag>`. To clear it, type `pm!s tag -clear`.");
            }
            else
            {
            ...
            async Task ShowIcon()
            {
                if ((ctx.System.AvatarUrl?.Trim() ?? "").Length > 0)
                {
                    var eb = new DiscordEmbedBuilder()
                        ...
                        .WithDescription("To clear, use `pm!system icon clear`.");
        ...
        public async Task SystemProxy(Context ctx)
        {
            ...
            {
                if (gs.ProxyEnabled)
                    await ctx.Reply("Proxying in this server is currently **enabled** for your system. To disable it, type `pm!disable`.");
                else
                    await ctx.Reply("Proxying in this server is currently **disabled** for your system. To enable it, type `pm!disable`.");
                return;
            }
         ...
         public async Task SystemTimezone(Context ctx)
        {
            ...
            {
                await ctx.Reply(
                    $"Your current system time zone is set to **{ctx.System.UiTz}**. It is currently **{SystemClock.Instance.GetCurrentInstant().FormatZoned(ctx.System)}** in that time zone. To change your system time zone, type `pm!s tz <zone>`.");
                return;
            }
        ...
        public async Task SystemPing(Context ctx) 
	    {
	        ...
	        if (!ctx.HasNext()) 
	        {
		        if (ctx.System.PingsEnabled) {await ctx.Reply("Reaction pings are currently **enabled** for your system. To disable reaction pings, type `pm!s ping disable`.");}
		        else {await ctx.Reply("Reaction pings are currently **disabled** for your system. To enable reaction pings, type `pm!s ping enable`.");}
	        }
            else {
