...
namespace PluralKit.Bot
{
    public class Member
    {
        ...
        public async Task NewMember(Context ctx) {
            ...
            await ctx.Reply($"{Emojis.Success} Member \"{memberName}\" (`{member.Hid}`) registered! ");
            if (await _db.Execute(conn => conn.QuerySingleAsync<bool>("select has_private_members(@System)",
                ...
                await ctx.Reply($"{Emojis.Warn} This member is currently **public**. To change this, use `pm!member {member.Hid} private`.");
