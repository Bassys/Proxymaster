    ...
    public class MemberEdit
    {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (target.Description == null)
                    if (ctx.System?.Id == target.System)
                        await ctx.Reply($"This member does not have a description set. To set one, type `pm!member {target.Reference()} description <description>`.");
                    else
                    ...
                    await ctx.Reply(embed: new DiscordEmbedBuilder()
                        ...
                        .AddField("\u200B", $"To print the description with formatting, type `pm!member {target.Reference()} description -raw`." 
                                    + (ctx.System?.Id == target.System ? $" To clear it, type `pm!member {target.Reference()} description -clear`." : ""))
                        .Build());
            }
            else
            {
        ...
        public async Task Pronouns(Context ctx, PKMember target) {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (target.Pronouns == null)
                    if (ctx.System?.Id == target.System)
                        await ctx.Reply($"This member does not have pronouns set. To set some, type `pm!member {target.Reference()} pronouns <pronouns>`.");
                    else
                        await ctx.Reply("This member does not have pronouns set.");
                else
                    await ctx.Reply($"**{target.NameFor(ctx)}**'s pronouns are **{target.Pronouns}**."
                        + (ctx.System?.Id == target.System ? $" To clear them, type `pm!member {target.Reference()} pronouns -clear`." : ""));
            }
            else
            {
                CheckEditMemberPermission(ctx, target);
        ...
        public async Task Color(Context ctx, PKMember target)
        {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (target.Color == null)
                    if (ctx.System?.Id == target.System)
                        await ctx.Reply(
                            $"This member does not have a color set. To set one, type `pm!member {target.Reference()} color <color>`.");
                    else
                    ...
                    await ctx.Reply(embed: new DiscordEmbedBuilder()
                        ...
                        .WithDescription($"This member's color is **#{target.Color}**."
                                         + (ctx.System?.Id == target.System ? $" To clear it, type `pm!member {target.Reference()} color -clear`." : ""))
                        .Build());
            }
            else
            {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (target.Birthday == null)
                    await ctx.Reply("This member does not have a birthdate set."
                        + (ctx.System?.Id == target.System ? $" To set one, type `pm!member {target.Reference()} birthdate <birthdate>`." : ""));
                else
                    await ctx.Reply($"This member's birthdate is **{target.BirthdayString}**."
                                    + (ctx.System?.Id == target.System ? $" To clear it, type `pm!member {target.Reference()} birthdate -clear`." : ""));
            }
            else
            {
                CheckEditMemberPermission(ctx, target);
        ...
        public async Task DisplayName(Context ctx, PKMember target)
        {
            ...
            else if (!ctx.HasNext())
            {
                ...
                if (ctx.System?.Id == target.System)
                    eb.WithDescription($"To change display name, type `pm!member {target.Reference()} displayname <display name>`.\nTo clear it, type `pm!member {target.Reference()} displayname -clear`.");
        ...
        public async Task ServerName(Context ctx, PKMember target)
        {
            ...
            else if (!ctx.HasNext())
            {
                ...
                var eb = await CreateMemberNameInfoEmbed(ctx, target);
                if (ctx.System?.Id == target.System)
                    eb.WithDescription($"To change server name, type `pm!member {target.Reference()} servername <server name>`.\nTo clear it, type `pm!member {target.Reference()} servername -clear`.");
                await ctx.Reply(embed: eb.Build());
            }
            else
            {
        ...
        public async Task Privacy(Context ctx, PKMember target, PrivacyLevel? newValueFromCommand)
        {
            ...
            async Task SetLevel(MemberPrivacySubject subject, PrivacyLevel level)
            {
                ...
                if (subject == MemberPrivacySubject.Avatar && level == PrivacyLevel.Private && guildSettings?.AvatarUrl == null)
                    await ctx.Reply($"{Emojis.Warn} This member does not have a server avatar set, so *proxying* will **still show the member avatar**. If you want to hide your avatar when proxying here, set a server avatar: `pm!member {target.Reference()} serveravatar`");
            }
