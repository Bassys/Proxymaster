    ...
    public class ServerConfig
    {
        ...
        public async Task SetLogEnabled(Context ctx, bool enable)
        {
            ...
            await ctx.Reply(
                ...
                (logChannel == null ? $"\n{Emojis.Warn} Please note that no logging channel is set, so there is nowhere to log messages to. You can set a logging channel using `pm!log channel #your-log-channel`." : ""));
        }
        ...
        public async Task SetLogCleanup(Context ctx)
        {
            ...
            else
            {
                ...
                if (guildCfg.LogCleanupEnabled)
                    eb.WithDescription("Log cleanup is currently **on** for this server. To disable it, type `pm!logclean off`."); 
                else 
                    eb.WithDescription("Log cleanup is currently **off** for this server. To enable it, type `pm!logclean on`.");
