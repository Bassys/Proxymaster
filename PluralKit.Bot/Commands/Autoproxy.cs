    ...
    public class Autoproxy
    {
        ...
        public async Task AutoproxyRoot(Context ctx)
        {
            ...
            if (ctx.Match("off", "stop", "cancel", "no", "disable", "remove"))
                ...
                throw new PKSyntaxError("Member-mode autoproxy must target a specific member. Use the `pm!autoproxy <member>` command, where `member` is the name or ID of a member in your system.");
        ...
        private async Task AutoproxyLatch(Context ctx)
        {
            if (ctx.MessageContext.AutoproxyMode == AutoproxyMode.Latch)
                await ctx.Reply($"{Emojis.Note} Autoproxy is already set to latch mode in this server. If you want to disable autoproxying, use `pm!autoproxy off`.");
            else
            {
        ...
        private async Task AutoproxyFront(Context ctx)
        {
            if (ctx.MessageContext.AutoproxyMode == AutoproxyMode.Front)
                await ctx.Reply($"{Emojis.Note} Autoproxy is already set to front mode in this server. If you want to disable autoproxying, use `pm!autoproxy off`.");
            else
            {
        ...
        {
            var commandList = "**pm!autoproxy latch** - Autoproxies as last-proxied member\n**pm!autoproxy front** - Autoproxies as current (first) fronter\n**pm!autoproxy <member>** - Autoproxies as a specific member";
            ...
            switch (ctx.MessageContext.AutoproxyMode) {
                ...
                case AutoproxyMode.Front:
                {
                    if (fronters.Length == 0)
                        eb.WithDescription("Autoproxy is currently set to **front mode** in this server, but there are currently no fronters registered. Use the `pm!switch` command to log a switch.");
                    else
                    {
                        ...
                        eb.WithDescription($"Autoproxy is currently set to **front mode** in this server. The current (first) fronter is **{relevantMember.NameFor(ctx).EscapeMarkdown()}** (`{relevantMember.Hid}`). To disable, type `pm!autoproxy off`.");
                    }
                ...
                case AutoproxyMode.Member when relevantMember != null: {
                    eb.WithDescription($"Autoproxy is active for member **{relevantMember.NameFor(ctx)}** (`{relevantMember.Hid}`) in this server. To disable, type `pm!autoproxy off`.");
                    break;
                }
                case AutoproxyMode.Latch:
                    eb.WithDescription("Autoproxy is currently set to **latch mode**, meaning the *last-proxied member* will be autoproxied. To disable, type `pm!autoproxy off`.");
                    break;
