...
namespace PluralKit.Bot
{
    ...
    {
        private static readonly TimeSpan MessageDeletionDelay = TimeSpan.FromMilliseconds(10);
        ...
        private async Task<bool> CheckBotPermissionsOrError(DiscordChannel channel)
        {
            ...
            if ((permissions & Permissions.ManageWebhooks) == 0)
            {
                ...
                await channel.SendMessageFixedAsync(
                    $"{Emojis.Error} Proxymaster does not have the *Manage Webhooks* permission in this channel. Be sure to turn it on!");
                return false;
            }
            ...
            {
                await channel.SendMessageFixedAsync(
                    $"{Emojis.Error} Proxymaster does not have the *Manage Messages* permission in this channel. Be sure to turn it on!");
                return false;
            }
