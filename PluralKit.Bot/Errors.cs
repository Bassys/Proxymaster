...
namespace PluralKit.Bot {
    ...
    public static class Errors {
		...
		public static PKError NoGroupError => new PKError("You do not have a group registered with that name. Make one with pm!g new!");
        public static PKError NoSystemError => new PKError("You do not have a system registered yet. To create one, type `pm!system new`!");
        public static PKError ExistingSystemError => new PKError("You already have a system registered with Proxymaster. To view it, type `pm!system`. If you'd like to delete your system and start anew, type `pm!system delete`, or if you'd like to unlink this account from it, type `pm!unlink`.");
        ...
        public static PKError AccountInOtherSystem(PKSystem system) => new PKError($"The mentioned account is already linked to another system (see `pm!system {system.Hid}`).");
        public static PKError UnlinkingLastAccount => new PKError("Since this is the only account linked to this system, you cannot unlink it (as that would leave your system account-less). If you would like to delete your system, use `pm!system delete`.");
        ...
        public static PKError InvalidImportFile => new PKError("Imported data file invalid. Make sure this is a .json file from a proxy bot.");
        public static PKError ImportCancelled => new PKError("Import cancelled.");
        public static PKError MessageNotFound(ulong id) => new PKError($"Message with ID '{id}' not found.");
        ...
        public static PKError LegacyAlreadyHasProxyTag(ProxyTag requested, PKMember member) => new PKError($"This member already has more than one proxy tag set: {member.ProxyTagsString()}\nConsider using the {$"pm!member {member.Reference()} proxy add {requested.ProxyString}".AsCode()} command instead.");
        public static PKError EmptyProxyTags(PKMember member) => new PKError($"The example proxy `text` is equivalent to having no proxy tags at all, since there are no symbols or brackets on either end. If you'd like to clear your proxy tags, use `pm!member {member.Reference()} proxy clear`.");

        public static PKError GenericCancelled() => new PKError("Operation cancelled.");

        public static PKError AttachmentTooLarge => new PKError("Proxymaster cannot proxy attachments over 8 megabytes (as webhooks aren't considered as having Discord Nitro) :(");
